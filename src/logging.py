...
class StructuredFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if hasattr(record, "operation_data"):
            ...
            if "file" in op_data:
                op_data["file"] = sanitize_path(
                    Path(op_data["file"]), self.privacy_mode
                )
...
class OperationMetrics:
    ...
    def record_operation(
        self, operation: str, size: int, duration: float, success: bool
    ):
        ...
        if not success:
            self.metrics["errors"][operation] = (
                self.metrics["errors"].get(operation, 0) + 1
            )
    ...
    def log_summary(self):
        ...
        self.logger.info(
            "Operation metrics summary", extra={"operation_data": self.metrics}
        )
